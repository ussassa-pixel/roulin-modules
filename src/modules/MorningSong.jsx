...
// 오늘을 여는 한 곡 (부록 morning_song) — 아침(pre-work) 시동용.
// music_pick과 곡 풀·저작권 규칙 공유, 진입 맥락이 다름:
//   music_pick = "지금 기분 대응"(기분 선택 단계 있음) / 이건 = "아침 시동"(선택 없이 바로 한 곡).
// 곡 선정: morning:true 플래그(energy 3~4, 각성 급점프 지양 — energy 5 제외).
//
// ⚖️ 저작권 4규칙 동일 적용(코드리뷰 항목): 재생 X · 가사 X · 앨범아트 X · 곡명/아티스트+링크아웃만.
...
export default function MorningSong({ onExit }) {
  ...
  const draw = () => {
    const morningPool = pool.songs.filter((s) => s.morning)
    let candidates = morningPool.filter((s) => !seenRef.current.has(s.id))
    if (candidates.length === 0) {
      seenRef.current.clear()
      candidates = morningPool
    }
  ...
  if (phase === 'intro')
    return page(
      <div className="max-w-md w-full text-center animate-fade-up">
        <p className="font-serif text-[28px] text-navy mb-3" style={{ fontWeight: 600 }}>오늘을 여는 한 곡</p>
        <div className="w-8 h-px bg-amber/60 mx-auto mb-4" />
        <p className="text-[14px] text-r-gray font-light mb-12 leading-relaxed">
          {isMorning ? <>좋은 아침이에요.<br />오늘을 여는 한 곡, 골라드릴게요.</> : <>오늘 하루의 배경음이 될<br />한 곡을 골라드릴게요.</>}
        </p>
        <button
          onClick={() => { setSong(draw()); setPhase('pick') }}
  ...
  if (phase === 'pick' && song) {
    ...
    return page(
      <div className="max-w-md w-full text-center animate-fade-up">
        <p className="eyebrow mb-8">오늘의 시작 곡</p>
        ...
        <button onClick={() => setSong(draw())} className="w-full py-3 text-[13px] text-r-gray-soft hover:text-r-gray tracking-wide transition">
          다른 곡으로
        </button>
      </div>
    )
