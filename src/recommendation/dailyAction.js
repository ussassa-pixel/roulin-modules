...
/**
 * 오늘의 행동 하나를 고른다. 결정적: 같은 (items, now의 날짜·timeband, recentIds) → 같은 결과.
 * @param {Array<{id:string,text:string,domain:string,timeband:string[],safety:string}>} items
 * @param {{ now?: Date, recentIds?: string[], crisisLevel?: 'none'|'L1'|'L2'|'L3' }} [opts]
...
export function pickDailyAction(items, opts = {}) {
  const { now = new Date(), recentIds = [], crisisLevel = 'none' } = opts
  ...
  const idx = hashStr(dateKey(now) + ':' + band) % eligible.length
  return eligible[idx]
}
