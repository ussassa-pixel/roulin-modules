...
const STEPS = [
  { key: 'person', q: '누구와의 일이에요?', hint: "이름 대신 '동료', '엄마'처럼 적어도 돼요.", type: 'input', ph: '예: 팀장님' },
  { key: 'situation', q: '어떤 일이 있었어요?', hint: '짧게, 한 장면이면 돼요.', type: 'area', ph: '그때의 한 장면을 적어봐요' },
  { key: 'myFeeling', q: '그때 나는 어땠어요?', hint: '떠오르는 대로.', type: 'input', ph: '예: 서운했어요' },
  { key: 'theirView', q: '잠깐 그 사람 자리에 서 본다면,\n그 사람에겐 어떤 사정이 있었을까요?', hint: '맞히지 않아도 돼요. 그냥 상상만.', type: 'area', ph: '상상해서 적어봐요' },
  { key: 'myWant', q: '이 관계에서 내가 정말 원하는 건\n뭘까요?', hint: '한 줄이면 충분해요.', type: 'input', ph: '예: 존중받고 싶어요' },
]
...
export default function RelationLens({ onExit }) {
  ...
  if (phase === 'step') {
    ...
    return page(
      <div key={step} className="max-w-md w-full animate-fade-in">
        <p className="text-center text-navy text-lg font-light mb-2 whitespace-pre-line leading-relaxed">{cur.q}</p>
        <p className="text-center text-r-gray-soft text-xs mb-8">{cur.hint}</p>
        {cur.type === 'area' ? (
          <textarea key={cur.key} className={taCls} rows={3} value={val} onChange={(e) => set(cur.key, e.target.value)} placeholder={cur.ph} autoFocus />
        ) : (
          <input key={cur.key} className={inputCls} value={val} onChange={(e) => set(cur.key, e.target.value)} placeholder={cur.ph} autoFocus />
        )}
        <button
          onClick={next}
          disabled={!val.trim()}
          className={`w-full py-4 rounded-full transition mt-5 ${val.trim() ? 'bg-navy text-white hover:bg-[#0c1a2b]' : 'bg-line text-r-gray-soft cursor-not-allowed'}`}
        >
