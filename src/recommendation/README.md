...
## recommender 스펙 반영 현황 (경로 3개, 공통 엔진)
- **경로 C "지금 어때요?" — 구현·UI 연결됨(1단계)**: `checkin.js`(상태 칩 7개 시그널 매핑 + 도구형 쿨다운 N=3일 DRAFT + 카테고리 브라우즈) + `components/CheckIn.jsx`(런처 히어로 CTA). 순수 규칙, LLM 0. 위기 표현은 칩에 없음 — "많이 힘들다면" 안전 화면은 엔진과 분리·상시 노출(문구 확정 = SW/Chaon 미결).
- **경로 B 시간대** — v4 time_slots(TimeSlotBanner)가 담당. 엔진 통합은 후속.
- **경로 A 대화 맥락** — 서비스 몫(`extractStateSignal` + verbatim 인용, 민감 발화 재노출 필터 미결).
- **④ 선택 규칙(엔진 공통)**: 최대 2개 / 3+ 동점(모호) → 1개 / 2파전 동점은 유형이 다를 때만 1+1 / 두 번째는 3점 미만(약한 매칭) 제외, 동점군에서 다른 유형 우선.
